...
use crate::runtime::streaming::operators::sink::filesystem::{
    FilesystemFormat, FilesystemSinkOperator, compression_from_str,
};
use crate::sql::common::constants::connection_format_value;
use crate::sql::common::with_option_keys as opt;

pub struct FilesystemSinkDispatcher;

impl OperatorConstructor for FilesystemSinkDispatcher {
    fn with_config(&self, payload: &[u8], _registry: Arc<Registry>) -> Result<ConstructedOperator> {
        ...
        let props = match op.config {
            Some(Config::FilesystemSink(cfg)) => filesystem_props(cfg),
            _ => bail!("filesystem sink expects FilesystemSinkConfig"),
        };
        ...
        let format = match format.as_str() {
            connection_format_value::CSV => FilesystemFormat::Csv,
            connection_format_value::PARQUET => FilesystemFormat::Parquet,
            connection_format_value::JSON => FilesystemFormat::JsonL,
            connection_format_value::AVRO => FilesystemFormat::Avro,
            connection_format_value::ORC => FilesystemFormat::Orc,
            other => bail!("unsupported filesystem sink format '{other}'"),
        };
        Ok(ConstructedOperator::Operator(Box::new(
            FilesystemSinkOperator::try_new(op.name, path, format, compression, sink_memory_bytes)?,
        )))
    }
}

fn filesystem_props(
    cfg: protocol::function_stream_graph::FilesystemSinkConfig,

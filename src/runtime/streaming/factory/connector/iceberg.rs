...
use crate::runtime::streaming::operators::sink::filesystem::compression_from_str;
use crate::runtime::streaming::operators::sink::iceberg::{IcebergFormat, IcebergSinkOperator};
use crate::sql::common::constants::connection_format_value;
use crate::sql::common::with_option_keys as opt;

pub struct IcebergSinkDispatcher;

impl OperatorConstructor for IcebergSinkDispatcher {
    fn with_config(&self, payload: &[u8], _registry: Arc<Registry>) -> Result<ConstructedOperator> {
        ...
        let props = match op.config {
            Some(Config::IcebergSink(cfg)) => iceberg_props(cfg),
            _ => bail!("iceberg sink expects IcebergSinkConfig"),
        };
        ...
        let format = match format.as_str() {
            connection_format_value::CSV => IcebergFormat::Csv,
            connection_format_value::PARQUET => IcebergFormat::Parquet,
            other => bail!("unsupported iceberg sink format '{other}'"),
        };
        Ok(ConstructedOperator::Operator(Box::new(
            IcebergSinkOperator::try_new(
                op.name,
                path,
                format,
                compression,
...
fn iceberg_props(
    cfg: protocol::function_stream_graph::IcebergSinkConfig,

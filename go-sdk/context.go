...
func (c *runtimeContext) Emit(targetID uint32, data []byte) error {
	...
	c.mu.RUnlock()
	if closed {
		return newError(ErrRuntimeClosed, "emit on closed context")
	}
...
func (c *runtimeContext) EmitWatermark(targetID uint32, watermark uint64) error {
	...
	c.mu.RUnlock()
	if closed {
		return newError(ErrRuntimeClosed, "emit watermark on closed context")
	}
	collector.EmitWatermark(targetID, watermark)
	return nil
}

func (c *runtimeContext) GetOrCreateStore(name string) (Store, error) {
	storeName := strings.TrimSpace(name)
	if storeName == "" {
		return nil, newError(ErrStoreInvalidName, "store name must not be empty")
	}

	c.mu.Lock()
	defer c.mu.Unlock()
	if c.closed {
		return nil, newError(ErrRuntimeClosed, "store request on closed context")
	}
